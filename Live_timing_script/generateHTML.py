...
def getHeaderDetailedRanking():
    ...
    tab_entete = '''
    <body style="background-image: url('BackGround.jpg'); background-size: contain; background-position: center; background-repeat: no-repeat; background-attachment: fixed; height: 100vh; margin: 0; display: flex; justify-content: center; align-items: center;">
        <table>
            <colgroup>
...
def getHeaderLightRanking():
    # Basic HTML structure with the head section
    tab_htmlbody = '<!DOCTYPE html><html>'
    tab_htmlbody += '<head><title>Light Ranking</title>'
    tab_htmlbody += '<meta charset="UTF-8">'
    
    # Add minimal script for AJAX if needed later
    tab_htmlbody += """
    <script>
        function updateTable() {
            var xhr = new XMLHttpRequest();
            xhr.open('GET', '/updateLightTable', true);  // Request light table content from the server
...
def getPilotLightRanking(pilot, showBestLap=False, showPilotCountryFlag=False, extended=False):
    style = 'style="color: lightgreen;"' if pilot.newPosition else ''
    lapTimeStyle = 'style="color: gold;"' if pilot.newBest else style
    NumberStyle = f'style="color: #fff;text-shadow:\
            1px 1px 0 #000,\
...
def generateLightTableHTML(pilots):
    ...
    tbody = "<tbody>"
    for pilot in pilots:
        tbody += getPilotLightRanking(pilot)
    tbody += "</tbody>"

    # Return thead and tbody as a JSON response for the AJAX function
    return {"thead": thead, "tbody": tbody}
